...
async function installAdGuardHome() {
  ...
  runCommand('sh', ['-c', 'curl -s -S -L https://raw.githubusercontent.com/AdguardTeam/AdGuardHome/master/scripts/install.sh | sh -s -- -v -r']);
...
function startQemu() {
  ...
  const qemu = spawn('qemu-system-x86_64', ['-m', '256M', '-drive', 'file=/root/alpine.qcow2,if=virtio,format=qcow2', '-boot', 'c', '-nic', `tap,ifname=${config.tapInterface},script=no,downscript=no,model=virtio-net-pci`, '-serial', 'mon:stdio', '-display', 'none'], { stdio: 'inherit' });
  qemu.on('exit', (code) => {
