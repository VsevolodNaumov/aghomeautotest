...
const config = {
    ...
    qemu: {
        ...
        commands: parseCloudInitCommands(process.env.CLOUDINIT_COMMANDS ?? 'echo "cloud-init provisioning completed"'),
    },
};
function parseCloudInitCommands(commands) {
    return commands
        .split(/\n|;/)
        .map((cmd) => cmd.trim())
        .filter(Boolean);
...
async function fetchDhcpInterfaces(cookie) {
...
async function enableDhcp(cookie) {
...
function prepareCloudInitImage() {
    ...
    if (!(0, fs_1.existsSync)(cloudInitDir)) {
        (0, fs_1.mkdirSync)(cloudInitDir, { recursive: true });
    }
    const userDataPath = `${cloudInitDir}/user-data`;
    const metaDataPath = `${cloudInitDir}/meta-data`;
    const runcmdSection = config.qemu.commands.length > 0
    ...
    (0, fs_1.writeFileSync)(userDataPath, userData, { encoding: 'utf-8' });
    (0, fs_1.writeFileSync)(metaDataPath, metaData, { encoding: 'utf-8' });
